...
%% Read ATACSeq dataset (subset of the Mouse sci-ATAC-seq Atlas)
input_path = '../../input/datasets/MouseBrain/ATAC';
fname = fullfile(input_path, 'preprocessed.mat');
if(~exist(fname, 'file'))
    ATACSeq.peaks = mmread(fullfile(input_path, 'peaks.mm'));
    ATACSeq.peak_annotations = readtable(fullfile(input_path, 'peak_annotations.bed'), 'Delimiter', '\t', 'FileType', 'text');
    ATACSeq.sample_annotations = readtable(fullfile(input_path, 'sample_annotations.txt'), 'Delimiter', '\t', 'FileType', 'text');
    save(fname, 'ATACSeq');
else
    load(fname);
end
Labels = ATACSeq.sample_annotations.cell_label;
UL = unique(Labels);
[~, labels] = ismember(Labels, UL);
%% Reduce chromatin accessibility profile
PCA_dim = 30;
method_no = 1;
tic; [S_r, ~, explained_var] = reduceChromatinAccessibility(ATACSeq.peaks, PCA_dim, method_no); toc
